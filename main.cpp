...
int main() {
  ...
  // Addition
  {
    ...
    usu::pound c = a + b;
    std::cout << std::format(
        "(pound + pound) : {:.2f} + {:.2f} = {:.2f} ==>
        grams
        : {}\n ",
          a.count(),
          b.count(), c.count(), usu::mass_cast<usu::gram>(c).count());
    usu::pound d{1};
    usu::pound before{d};
    d += b;
    std::cout << std::format(
        ...
        : {}\n ",
          before.count(),
          b.count(), d.count(), usu::mass_cast<usu::gram>(d).count());
    ...
    std::cout << std::format(
        ...
        : {}\n ",
          before.count(),
          e.count(), f.count(), usu::mass_cast<usu::gram>(f).count());
  }
  // Subtraction
  {
    std::cout << std::endl;
  ...
  // Scalar multiplication
  {
    ...
    usu::pound c = 3.2 * a;
    std::cout << std::format(
        "(pound * scalar) : {:.2f} * 2.20 = {:.2f} ==>
        grams
        : {}\n ",
          a.count(),
          b.count(), usu::mass_cast<usu::gram>(b).count());
    std::cout << std::format(
        "(scalar * pound) : 3.20 * {:.2f} = {:.2f} ==>
        grams
        : {}\n ",
          a.count(),
          c.count(), usu::mass_cast<usu::gram>(c).count());
    usu::pound d{1.5};
    usu::pound before{d};
    d *= 2;
    std::cout << std::format(
        "(pound *= scalar) : {:.2f} *= 2 = {:.2f} ==>
        grams
        : {}\n ",
          before.count(),
          d.count(), usu::mass_cast<usu::gram>(d).count());
  }
  // Scalar division
  {
    std::cout << std::endl;
    ...
    usu::gram e = c / 4;
    std::cout << std::format(
        "(gram / scalar) : {0:d} / 2 = {1:d} ==> grams:
        { 1 : d }\n ",
        c.count(),
        d.count());
    std::cout << std::format(
        "(gram / scalar) : {0:d} / 4 = {1:d} ==> grams:
        { 1 : d }\n ",
        c.count(),
        e.count());
    usu::pound f{3};
    ...
    std::cout << std::format(
        ...
        : {}\n ",
          before.count(),
          f.count(), usu::mass_cast<usu::gram>(f).count());
  }
  // Adding mixed types
  {
    std::cout << std::endl;
    ...
    std::cout << std::format("(gram + pound + ounce) : {:d} + {:.6f} + {:.6f} =
                             { : d} grams\n ",
                             g.count(),
                             lb.count(), oz.count(), gramResult.count());
    std::cout << std::format("(ounce + gram + pound) : {:.6f} + {:d} + {:.6f} =
                             { : .6f} ounces\n ",
                             oz.count(),
                             g.count(), lb.count(), ozResult.count());
    std::cout << std::format("(pound + gram + ounce) : {:.6f} + {:d} + {:.6f} =
                             { : .6f} pounds\n ",
                             lb.count(),
                             g.count(), oz.count(), lbResult.count());
